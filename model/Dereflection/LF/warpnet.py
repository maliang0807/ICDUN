...
from model.Dereflection.LF.disparity_utils import warp_to_ref_view_parallel
...
#         tempV = []
#         for j in range(w):
#             tempV.append(x[:, :, i::h, j::w].clone())
#         tempU.append(torch.cat(tempV, dim=3))
#     out = torch.cat(tempU, dim=2)
#     return out
def SAI2MacPI(x, angRes):
    B, C, H, W = x.shape
    assert H % angRes == 0 and W % angRes == 0, "H, W must be divisible by angRes"
    h, w = H // angRes, W // angRes  # 每个小视图的尺寸

    out = torch.zeros(B, C, h * angRes, w * angRes, device=x.device, dtype=x.dtype)
    for i in range(angRes):
        for j in range(angRes):
            out[:, :, i*h:(i+1)*h, j*w:(j+1)*w] = x[:, :, i::angRes, j::angRes]
    return out
